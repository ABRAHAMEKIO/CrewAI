import MidjourneyClient, { SuccessResponse } from "../../../domain/midjourney/midjourneyClient";
...
export default async function handler(req, res) : Promise<SuccessResponse> {
  const { body: { msg, ref } } = req;

  const midjourneyClient = new MidjourneyClient(AUTH_SECRET);
  const imageCommandResponse = await midjourneyClient.imagine(msg, ref, WEBHOOK_OVERRIDE || "");
